...
void highlight_write2sock(UR_OBJECT user,char *buff,size_t num)
{
	...
	if (user->findstr[0] && strstr(buff,user->findstr) && user->colour) {
		...
		while (tmp2!=NULL) {
			...
			if (web) strncpy(tmp2,"~BR",3);
			else strncpy(tmp2,"\033[41m",5);
			tmp2+=strlen(user->findstr)+5-web*2;
			memmove(tmp2+5-web*2,tmp2,strlen(tmp2)+1);
			if (web) strncpy(tmp2,"~BR",3);
			else strncpy(tmp2,"\033[40m",5);
			tmp=tmp2+5-web*2;
			tmp2=strstr(tmp,user->findstr);
		}
			...
			switch (user->browsing) {
				case 2:
					if (user->findstr[0]) {
						...
						if (max==0) sprintf(text,"Nemas ziadnu postu.\n");
						else sprintf(text,"V schranke mas len %d %s.\n",max,skloncislo(max,"spravu","spravy","sprav"));
					}
					write_user(user,text);
					break;
				case 10:
...
void wipe_board(UR_OBJECT user)
{
	...
	if (toid==0) sprintf(query,"delete from `board` where `room=`'%s' and `deleted`=0 and%s %d<=`msgid`",user->room->name,texthb,fromid);
	else sprintf(query,"delete from `board` where `room`='%s' and `deleted`=0 and%s %d<=`msgid` and `msgid`<=%d",user->room->name,texthb,fromid,toid);
	mysql_kvery(query);
	vyfluslo=mysql_affected_rows(&mysql);
	sprintf(query,"select count(`msgid`) from `board` where `room`='%s' and `deleted`=0",user->room->name);
	cnt=query_to_int(query);
