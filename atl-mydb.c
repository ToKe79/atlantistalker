...
extern int db_save_user_details(UR_OBJECT user, int save_current)
{
   ...
   if (save_current==1) online=1;
   if (save_current==8) save_current=1;
   ...
   /* Zlozi ignore cislo */
   user->ignore=0;
   if (user->ignshout) user->ignore+=1;
   if (user->igntell) user->ignore+=2;
   if (user->ignall) user->ignore+=4;
   if (user->ignsys) user->ignore+=8;
   if (user->ignfun) user->ignore+=16;
   if (!user->igngossip) user->ignore+=32;
   if (user->ignportal) user->ignore+=64;
   if (user->ignio) user->ignore+=128;
   if (user->ignlook) user->ignore+=256;
   if (user->ignbeep) user->ignore+=512;
   if (user->ignzvery) user->ignore+=1024;
   if (user->igncoltell) user->ignore+=2048;
   if (!user->ignportalis) user->ignore+=4096;
   if (!user->ignafktell) user->ignore+=8192;
   if (user->mailnotify) user->ignore+=16384;
   if (user->igngames) user->ignore+=32768;
   if (user->ignautopromote) user->ignore+=65536;
   if (user->ignxannel) user->ignore+=131072;
   if (user->ignnongreenhorn) user->ignore+=262144;

   user->switches=0;
   if (user->shortcutwarning) user->switches+=1;
   if (user->misc_op==33) user->switches+=2;
   if (user->mail2sms) user->switches+=4;
   if (user->com_priority) user->switches+=8;
   if (user->timeinrevt) user->switches+=16;
   if (user->ignblink) user->switches+=32;
   if (user->ignspell) user->switches+=64;
   if (user->ignpict) user->switches+=128;
/*   if (user->) user->switches+=256;
   if (user->) user->switches+=512;
   if (user->) user->switches+=1024;
   if (user->) user->switches+=2048;
   if (user->) user->switches+=4096;
   if (user->) user->switches+=8192;
   if (user->) user->switches+=16384;
   if (user->) user->switches+=32768;
*/
   ...
   if (user->doom) user->lines+=D_LINES; /* D00M - o D_LINES je menej */
   if (user->statlcount) user->lines=user->statlcount; /* statline */
 ...
 '%d','%d','%d','%d','%d',\
 '%d','%d','%d','%d','%d',\
 FROM_UNIXTIME(%d),'%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d',\
 '%d','%d','%d','%d','%d','%d','%d','%d',\
 '%d','%d','%d','%d','%d','%d','%d','%d','%d','%d',\
 '%d','%d','%d','%d','%d','%d','%d','%d',\
 '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%d',\
 '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');",\
      save_current==2?"repository":"users",uid[0]?"id,":"",uid,
      user->name,ppass,
      lastlogin,(int)user->total_login,lastloginlen,(int)user->read_mail,user->level,user->prompt,user->muzzletime,user->charmode_echo,user->command_mode,user->colour,user->jailed,user->sex,user->pp,user->gold,user->sayswears,
      user->ignore,user->lines,user->wrap,user->who_type,user->rt_on_exit,
      user->lastm,user->autofwd,user->mana,user->pagewho,user->examine,
      (int)user->first_login,user->col[0],user->col[1],user->col[2],user->col[3],user->col[4],user->col[5],user->col[6],user->lang,user->goafkafter,user->killmeafter,ch,
      user->commused[0],user->commused[1],user->commused[2],user->commused[3],user->commused[4],user->commused[5],user->commused[6],user->commused[7],user->commused[8],user->commused[9],
      user->totaljailtime,user->totalmuzzletime,user->nontimejails,user->killed,user->commused[10],user->muzzled,user->team,
      user->shoutswears,user->smsssent,user->smsday,user->allowsms,user->switches,user->smsgate,user->agecode,(int)user->longestsession,(int)user->idletime,
      lastsite,pdesc,pinphr,poutphr,user->email,phomepage,pprompt,user->room->name,user->requestemail,user->mobile,pwizpass,user->smsfollow,plogoutmsg,
      online,user->prevname,user->shoutmsg,user->shoutmsg2,user->gossipmsg,user->gossipmsg2,user->saymsg,user->saymsg2,
      user->tellmsg,user->tellmsg2,user->wizshmsg,user->wizshmsg2,user->request,user->sname[0],user->sname[1],user->sname[2]
      );
   ...
   if (user->statlcount) user->lines-=2; /* statline */
   if (user->doom) user->lines-=D_LINES; /* Obnoviime... */
   return 1;
}
...
int i;
   ...
   if (pohaluz(user)!= atoi(row[35])) user->gold=0; /* V) Okradli usera! parchanti! ;) */
   ...
   if (user->goafkafter<3 || user->goafkafter>10) user->goafkafter=10;
   if (user->killmeafter<10 || user->killmeafter>120) user->killmeafter=120;
   if (user->killed<0) user->killed=0;
   if (user->totaljailtime<0) user->totaljailtime=0;
   if (user->nontimejails<0) user->nontimejails=0;
   if (user->col[5]<0) user->col[5]=0;
   if (user->muzzled<0) user->muzzled=0;
   if (user->team<0 || user->team>4) user->team=0;
   if (teamname[user->team][0]=='\0') user->team=0;
   if (user->allowsms==-1) user->allowsms=0;
   if (user->smsgate<0) user->smsgate=0;
   if (user->switches<0) user->switches=0;
   
   if (user->ignore>=262144) { user->ignnongreenhorn=1; user->ignore-=262144; }
   if (user->ignore>=131072) { user->ignxannel=1; user->ignore-=131072; }
   if (user->ignore>=65536) { user->ignautopromote=1; user->ignore-=65536; }
   if (user->ignore>=32768) { user->igngames=1; user->ignore-=32768; }
   if (user->ignore>=16384) { user->mailnotify=1; user->ignore-=16384; }
   if (user->ignore>=8192) { user->ignafktell=0; user->ignore-=8192; }
   if (user->ignore>=4096) { user->ignportalis=0; user->ignore-=4096; }
   if (user->ignore>=2048) { user->igncoltell=1; user->ignore-=2048; }
   if (user->ignore>=1024) { user->ignzvery=1; user->ignore-=1024; }
   if (user->ignore>=512) { user->ignbeep=1; user->ignore-=512; }
   if (user->ignore>=256) { user->ignlook=1; user->ignore-=256; }
   if (user->ignore>=128) { user->ignio=1; user->ignore-=128; }
   if (user->ignore>=64) { user->ignportal=1; user->ignore-=64; }
   if (user->ignore>=32) { user->igngossip=0; user->ignore-=32; }
   if (user->ignore>=16) { user->ignfun=1; user->ignore-=16; }
   if (user->ignore>=8) { user->ignsys=1; user->ignore-=8; }
   /* toto nje dobra napad ignore savovat all.. akurat na zabudnem to ked
    zajtra seba nalogujem a budem cudovat ze co ma setci v pazi haben.. (V) */
   if (user->ignore>=4) { user->ignall=1; user->ignore-=4; }
   user->ignall=0;
   
   if (user->ignore>=2) { user->igntell=1; user->ignore-=2; }
   if (user->ignore>=1) { user->ignshout=1; user->ignore-=1; }

/* if (user->switches>=32768) { user->=1; user->switches-=32768; }
   if (user->switches>=16384) { user->=1; user->switches-=16384; }
   if (user->switches>=8192)  { user->=1; user->switches-=8192; }
   if (user->switches>=4096)  { user->=1; user->switches-=4096; }
   if (user->switches>=2048)  { user->=1; user->switches-=2048; }
   if (user->switches>=1024)  { user->=1; user->switches-=1024; }
   if (user->switches>=512)   { user->=1; user->switches-=512; }
   if (user->switches>=256)   { user->=1; user->switches-=256; }*/
   if (user->switches>=128)   { user->ignpict=1; user->switches-=128; }
   if (user->switches>=64)    { user->ignspell=1; user->switches-=64; }
   if (user->switches>=32)    { user->ignblink=1; user->switches-=32; }
   if (user->switches>=16)    { user->timeinrevt=1; user->switches-=16; }
   if (user->switches>=8)     { user->com_priority=1; user->switches-=8; }
   if (user->switches>=4)     { user->mail2sms=1; user->switches-=4; }
   if (user->switches>=2)     { user->wizactivity=10; user->switches-=2; }
   if (user->switches>=1)     { user->shortcutwarning=1; user->switches-=1; }
   ...
   if ((user->room=get_room(row[70],NULL))==NULL) /* R00M */
   ...
   if (user->agecode>1) user->age=get_age(user->agecode);
   else user->age=0;
...
int uvoted[101],cnt=0,i;
 ...
 if (word_count==1) {
   ...
   while ((row=mysql_fetch_row(result))) {
     if (row[0]!=NULL) {
       ...
       if (closed==0) {
         for(i=0;i<cnt;i++)
          if (id==uvoted[i]) already_voted=1;
        }
   ...
   mysql_free_result(result);
   if (total==0) write_user(user,"Momentalne neprebieha ziadne hlasovanie.\n");
   else write_user(user,"~OL~FY+------------------------------------------------------------------------------+\n");
   return;
  }
 ...
 if (word_count==2) {
   ...
   if((row=mysql_fetch_row(result))) {
     id=atoi(row[0]);
     if (row[1]) strncpy(text,row[1],T_SIZE-10);
     if (row[2]) strncpy(label,row[2],20);
     closed=atoi(row[3]);
     label[20]='\0';
     mysql_free_result(result);
    }
   ...
   if (!closed) {
     ...
     if (mysql_num_rows(result)) already_voted=1;
     mysql_free_result(result);
    }
   else already_voted=1;
 ...
 if (word_count==3) {
   ...
   if((row=mysql_fetch_row(result))) {
     id=atoi(row[0]);
     if (row[1]) strncpy(text,row[1],T_SIZE);
     if (row[2]) strncpy(label,row[2],20);
     closed=atoi(row[3]);
     label[20]='\0';
     mysql_free_result(result);
...
if (type_def==0) {
  if (word_count<2) {
    write_user(user,"Pouzi: .winners <arena | piskvorky | hangman | doom | letun\n"
  	          "                 quest | reversi | dama | miny> [n]\n       [n] moze byt cislo od 1 po 500, standardne 10.\n");
    return;	              
   }
  if (!(kolko=atoi(word[2]))) kolko=10;
  if (kolko>500) {
    write_user(user,"Cislo musi byt z rozsahu 1-500!\n");
    return;
   }
  strtolower(word[1]);
  if (!strncmp(word[1],"ar",2)) { type=DB_ARENA; strcpy(label,"bojovnikov"); }
  if (!strncmp(word[1],"pi",2)) { type=DB_PISKVORKY; strcpy(label,"piskvorkarov"); } 
  if (!strncmp(word[1],"ha",2)) { type=DB_HANGMAN; strcpy(label,"obesencov"); }
  ...
  if (!strncmp(word[1],"br",2)) { type=DB_BRUTALIS;strcpy(label,"masovych vrahov"); } 

  if (type==0) {
    write_user(user,"Pouzi: .winners <arena | piskvorky | hangman | doom | letun\n"
                    "                 quest | reversi | dama | miny> [n]\n       [n] moze byt cislo od 1 po 99, standardne 10.\n");               
    return;
   }
 }
else {
  ...
  kolko=pocet;
      ...
      if (type==DB_ARENA)     { strcpy(lab,"arena");strcpy(label,"bojovnikov"); mode=1; }
 else if (type==DB_QUEST)     { strcpy(lab,"quest");strcpy(label,"questerov"); mode=1; }
 else if (type==DB_PISKVORKY) { strcpy(lab,"pisk"); strcpy(label,"piskvorkarov"); }
 else if (type==DB_HANGMAN)   { strcpy(lab,"hang"); strcpy(label,"obesencov"); }
 else if (type==DB_DOOM)      { strcpy(lab,"doom"); strcpy(label,"zabijakov"); }
 else if (type==DB_LETUN)     { strcpy(lab,"fly");  strcpy(label,"pilotov"); mode=2; } 
 else if (type==DB_REVERSI)   { strcpy(lab,"reve"); strcpy(label,"reverzistov"); }
 else if (type==DB_DAMA)      { strcpy(lab,"dama"); strcpy(label,"hracov damy"); }
 else if (type==DB_MINY)      { strcpy(lab,"miny"); strcpy(label,"hladacov"); }
 ...
 while ((row=mysql_fetch_row(result))) {
   if (row[0]!=NULL) {
     ...
     if (i==1) { 
       ...
       write_user(user,text);
       if (mode==1) write_user(user,"~OL~FB| ~FWtop~FB |     ~FWmeno~FB     |   ~FWskore ~FB   |\n~OL~FB|=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=|\n");
       else if (mode==2) write_user(user,"~OL~FB| ~FWtop~FB | ~FWmeno pilota~FB  | ~FWuspech~FB | ~FWneuspech~FB | ~FWpal. ~FB| ~FWskore ~FB|\n~OL~FB|=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=|\n");
       else write_user(user,"~OL~FB| ~FWtop~FB |     ~FWmeno~FB     | ~FWvyhry~FB | ~FWprehry~FB | ~FWskore ~FB|\n~OL~FB|=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=|\n");
      }
     if (mode==1) sprintf(text,"~OL~FB|~RS~FW%4d ~OL~FB| ~FY%s%-12s~FB | ~FW%7s~FB    |\n",i,strcmp(row[0],user->name)?"":"~FR",row[0],row[1]);
     else if (mode==2) sprintf(text,"~OL~FB|~RS~FW%4d ~OL~FB| ~FY%s%-12s~FB |~RS~FW%6s~OL~FB  |~RS~FW%7s~OL~FB   |~RS~FW%5s~OL~FB |~FW%6s ~FB|\n",i,strcmp(row[0],user->name)?"":"~FR",row[0],row[1],row[2],row[3],row[4]);
     else sprintf(text,"~OL~FB|~RS~FW%4d ~OL~FB| ~FY%s%-12s~FB |~RS~FW%6s~OL~FB |~RS~FW%7s~OL~FB |~FW%6d~FB |\n",i,strcmp(row[0],user->name)?"":"~FR",row[0],row[1],row[2],atoi(row[1])-atoi(row[2]));
     write_user(user,text);
     if (!strcmp(row[0],user->name)) current=1;
    }
  }
 mysql_free_result(result);
 if (i==0) write_user(user,"Tabulka je prazdna.\n");
 else {
   if (mode==1) write_user(user,"~OL~FB|=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=|\n");
...
{
int wins=0,losts=0;
int arena=0,quest=0,pisk_w=0,pisk_l=0,hang_w=0,hang_l=0,doom_w=0,doom_l=0,
reve_w=0,reve_l=0,dama_w=0,dama_l=0,miny_w=0,miny_l=0,brut_w=0,brut_l=0,fly_w=0,fly_l=0,fly_p=0,fly_s=0;
 ...
 if      (type==DB_PISKVORKY) { pisk_w+=wins; pisk_l+=losts; }
 else if (type==DB_HANGMAN)   { hang_w+=wins; hang_l+=losts; }
 else if (type==DB_DOOM)      { doom_w+=wins; doom_l+=losts; }
 else if (type==DB_REVERSI)   { reve_w+=wins; reve_l+=losts; }
 else if (type==DB_DAMA)      { dama_w+=wins; dama_l+=losts; }
 else if (type==DB_MINY)      { miny_w+=wins; miny_l+=losts; }
 else if (type==DB_BRUTALIS)  { brut_w+=wins; brut_l+=losts; }
 else if (type==DB_ARENA)     { arena+=wins; }
 else if (type==DB_QUEST)     { quest+=wins; }
 else if (type==DB_LETUN)     {
   fly_w+=wins;
   fly_l+=losts;
   if (fuel>fly_p) fly_p=fuel;
   if ((fly_p-90)<0) fly_s=fly_w-fly_l;
...
char inpstr[ARR_SIZE+1];
 ...
 while (user!=NULL) {
   ...
   if (user->websecond!=tsec) {
     ...
     if (inpstr[0]) got_line(user,inpstr);
    }
...
/* int kick[50]; */
 ...
 if (cnt) for (i=0;i<cnt;i++) {
   ...
   while (user!=NULL) {
     next=user->next;
     if (user->type!=USER_TYPE || user->socket<1000 || (int)(time(0)-user->last_input)<30) { user=next; continue; }
     if (user->socket!=id[i]+1000) { user=next; continue; }
     if (user->login) {
...
if (word_count==1) {
  ...
  if (word_count>3) {
    ...
    record_tell(u,text);
    sprintf(text,"%s si %s zvoleny obrazok: %s\n",pohl(user,"Poslal","Poslala"), sklonuj(u, 3),inpstr);
    write_user(user,text);
    sprintf(text,"%s si ~OL~FW\252C3%s~RS~FW obrazok: %s\n",pohl(user,"Poslal","Poslala"), sklonuj(u, 3),inpstr);
    ...
    if (word_count>2) {
      ...
      record_tell(u,text);
      sprintf(text,"%s si ~OL~FW\252C3%s~RS~FW zvoleny obrazok.\n",pohl(user,"Poslal","Poslala"), sklonuj(u, 3));
      write_user(user,text);
      sprintf(text,"%s si ~OL~FW\252C3%s~RS~FW obrazok..\n",pohl(user,"Poslal","Poslala"), sklonuj(u, 3));
      record_tell(user,text);
     } 
   }
 }
...
{ if (user->socket>1000) return 1; else return 0; }
...
int web,cnt=0;
  ...
  if (user->findstr[0] && strstr(buff,user->findstr) && user->colour) {
    ...
    while (tmp2!=NULL) {
      ...
      if (web) strncpy(tmp2,"~BR",3); else strncpy(tmp2,"\033[41m",5);
      tmp2+=strlen(user->findstr)+5-web*2;
      memmove(tmp2+5-web*2,tmp2,strlen(tmp2)+1);
      if (web) strncpy(tmp2,"~BR",3); else strncpy(tmp2,"\033[40m",5);
      tmp=tmp2+5-web*2;
      tmp2=strstr(tmp,user->findstr);
     }
...
PAGER pom;
 ...
 if ((result=mysql_result(query))) {
   if (!(row=mysql_fetch_row(result))) eor=1;
   fetch++;
   if (user->findstr[0]) {
     while (!eor && !strstr(row[0],user->findstr)) {
       if (!(row=mysql_fetch_row(result))) eor=1;
       else { fetch++; user->messnum++; }
       if (fetch==max && eor) {
         fetch=1;
         user->messnum++;
         construct_query(user,max);
         mysql_free_result(result);
         if ((result=mysql_result(query))) {
           if ((row=mysql_fetch_row(result))) { eor=0; }
          }
        }
      }
    }
 ...
 if (user->filepos<=strlen(row[0])) pos=user->filepos;
 else pos=0;
       ...
       /* if (lines==0 || (lines)%totalines!=0) { */
        {
         if (!silent) {
           switch(user->browsing) {
             ...
             case 11:
             case 12:
              if (row[3] && atoi(row[3])>=user->read_mail) { i=1; user->read_mail=atoi(row[3])+1; }
              else i=0;
              if (user->colour) {
                if (webuser) {
                  ...
                  sprintf(text,"%s===%s%s\033[1m\033[37m%d%s===\033[0m\033[33mOd: \033[1m\033[37m%s \033[0m\033[33mprijata \033[1m\033[37m%s %s======================================",
                  i?"\033[1m\033[36m":"\033[1m\033[34m",user->messnum>99?"":"=",user->messnum>9?"":"=",user->messnum,i?"\033[1m\033[36m":"\033[1m\033[34m",row[1]?row[1]:"",row[2]?sqldatum(row[2],2,0):"??.??.???? ??:??",i?"\033[1m\033[36m":"\033[1m\033[34m");
                  text[80+98]='\0';
                  strcat(text,"\r\n\033[0m\033[37m");
                 }
               }
             ...
             case 21:
               ...
               if (user->colour) {
                 if (webuser) {
                   sprintf(text,"~HB=-=-=%s%s%s%s~HW%d~HB=-=-=-=-=-=-=-=-=-=~DG Vtip napisal: ~HY%s ~HB=-=-=-=-=-=-=-=-=-=-=-=-",
                   user->messnum>9999?"":"-",user->messnum>999?"":"=",user->messnum>99?"":"-",user->messnum>9?"":"=",user->messnum,row[1]?row[1]:"Neznamy");
                   text[80+5]='\0';
                   sprintf(texthb,"%s%s%s%s~HW%d~HB=-=-=-=\r\n~RS~FW",i>9999?"":"-",i>999?"":"=",i>99?"":"-",i>9?"":"=",i);
                   strcat(text,texthb);
                  }
                 else {
                   sprintf(text,"\033[1m\033[34m=-=-=%s%s%s%s\033[1m\033[37m%d\033[1m\033[34m=-=-=-=-=-=-=-=-=-=\033[0m\033[32m Vtip napisal: \033[1m\033[33m%s \033[1m\033[34m=-=-=-=-=-=-=-=-=-=-=-=-",
                   user->messnum>9999?"":"-",user->messnum>999?"":"=",user->messnum>99?"":"-",user->messnum>9?"":"=",user->messnum,row[1]?row[1]:"Neznamy");
                   text[80+41]='\0';
                 ...
                 sprintf(text,"=-=-=%s%s%s%s%d=-=-=-=-=-=-=-=-=-= Vtip napisal: %s =-=-=-=-=-=-=-=-=-=-=-=-",
                 user->messnum>9999?"":"-",user->messnum>999?"":"=",user->messnum>99?"":"-",user->messnum>9?"":"=",user->messnum,row[1]?row[1]:"Neznamy");
                 text[67]='\0';
             ...
             if (user->browsing>2) {
              ...
              i=0;
              if (row[4]) {
                if (row[4][strlen(row[4])-1]=='1') strcpy(texthb,"Neznamy");
                else if (row[4][strlen(row[4])-1]=='2') strcpy(texthb,"Neznama");
         ...
         lines++;
         if (lines>0 && (lines)%totalines==0) continue;
        }
    }
   ...
   while (row[0][pos]) {
     ...
     if (row[0][pos]=='\n') {
       if (silent) {
         ...
         *(buff+buffpos)='\0';
         if (user->ignword[0] && strstr(buff,user->ignword)) buffpos=buffold;
       ...
       if (row[0][pos]>=127) row[0][pos]=' '; /* UMBA */
	   ...
	   if (row[0][pos]==colcom2[i][0] && row[0][pos+1]==colcom2[i][1]) {
             ...
             if (!silent) {
  	       if (user->colour && !(user->colour==2 && !strcmp(colcom2[i],"LI"))
               && !(user->ignbeep && !strcmp(colcom2[i],"LB"))
               && !(user->ignblink && !strcmp(colcom2[i],"LI"))) {
 ...
 if (eor) {
   ...
   else write_user(user,"~FM====+~OL+~RS~FM+==================================================================+~OL+~RS~FM+====\n");
   user->filepos=0;
   user->messsize=0;
   no_prompt=0;
   retval=2;
   ...
   if (user->messsize) fperc=(int)(i*100)/user->messsize;
   else fperc=0;
   fperc=100-fperc;
...
/*** Write on the message board 
	Zmena Spakky - teraz vypisuje inak, viacmenej prehladnejsie. ***/
...
if (user->muzzled) { write_user(user,"Si umlcany, a preto nemozes pisat na nastenku!\n"); return; }
...
if ((word_count==2) && (from>=1)) to=from; /* Uprava -> iba JEDEN message */
...
void write_joke(user,inpstr,done_editing)
UR_OBJECT user;
char *inpstr;
int done_editing;
{
...
write_user(user,text);
sprintf(text,"~OL~FT:^) ~RS~FG%s %s vtip na JokeBoard!~OL~FT :^)\n",user->name, pohl(user,"napisal","napisala"));
write_room_except(NULL,text,user);
pocet_vtipov+=1;
}

/* ZMENA Spakky - Prikaz na precitanie JokeBoardu */
void read_joke(user)
UR_OBJECT user;
{
char *name;
...
if (word_count>1) {
  ...
  if (user->rjoke_from<1) user->rjoke_from=1;
 }
else user->rjoke_from=1;
...
/* ZMENA Spakky - Spocita vtipy na jokeboarde. */
int countjoke()
{
int cnt;
...
int mail_from(user,echo) /* vrati pocet sprav (V) */
UR_OBJECT user;
int echo;
{
...
if (word[1][0]=='n') {
  ...
  if (!cnt) {
    if (echo) write_user(user,"Nemas ziadnu postu.\n");
    return 0;
   }
  else {
...
/* llang */
if (user->lang) write_user(user,"~OL~FWMail sent.~RS~FW\n");
else write_user(user,"~OL~FWPosta odoslana.~RS~FW\n");
...
else if (ret==1) user->misc_op=223;
if (user->vis) name=user->name; else name=invisname(user);
if (rm==user->room) {
...
if ((word_count==2) && (from>=1)) to=from;
...
if (cnt==0) {
  ...
  sprintf(text,"%d %s, zostalo %d.\n",vyfluslo,skloncislo(vyfluslo,"vtip bol zmazany","vtipy boli zmazane","vtipov bolo zmazanych"),cnt);
  user->lastm=cnt;
  write_user(user,text);
 }
}
