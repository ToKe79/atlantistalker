...
void destruct_vec(P_OBJECT vec)
{
	...
	if (vec->leave!=NULL)
		free((void *)vec->leave);
	if (vec->uinphr!=NULL)
		free((void *)vec->uinphr);
	if (vec->uoutphr!=NULL)
		free((void *) vec->uoutphr);
	if (vec->rinphr!=NULL)
		free((void *) vec->rinphr);
	if (vec->routphr!=NULL)
		free((void *) vec->routphr);
	if (vec->ustart!=NULL)
		free((void *) vec->ustart);
	if (vec->rstart!=NULL)
		free((void *) vec->rstart);
	if (vec->ustop!=NULL)
		free((void *) vec->ustop);
	if (vec->rstop!=NULL)
		free((void *) vec->rstop);
	if (vec->userphr!=NULL)
		free((void *) vec->userphr);
	if (vec->roomphr!=NULL)
		free((void *) vec->roomphr);
	if (vec->victimphr!=NULL)
		free((void *) vec->victimphr);
	if (vec->special!=NULL)
		free((void *) vec->special);
	if (vec->error!=NULL)
		free((void *) vec->error);
	if (vec->udestroy!=NULL)
		free((void *) vec->udestroy);
	if (vec->rdestroy!=NULL)
		free((void *) vec->rdestroy);
	if (vec->ujoinphr!=NULL)
		free((void *) vec->ujoinphr);
	if (vec->rjoinphr!=NULL)
		free((void *) vec->rjoinphr);
	if (vec->searchphr!=NULL)
		free((void *) vec->searchphr);
	free(vec);
}
...
{
	if (con->usermsg!=NULL)
		free((void *)con->usermsg);
	if (con->roommsg!=NULL)
		free((void *)con->roommsg);
	if (con->missing!=NULL)
		free((void *)con->missing);
	free((void *)con);
}
...
{
	int num,total,i,p,maxbefore=0,index;
	RM_OBJECT rm;
	UR_OBJECT u;
	...
	while ((row=mysql_fetch_row(result))) {
		...
		if (strlen(row[num])==0)
			predmet[total]->spawn=NULL;
		else
			predmet[total]->spawn=get_room(row[num],NULL);
		num++;
	...
	if (user!=NULL) {
		for(i=0;i<maxbefore;i++) {
			...
			while (predmet[p]!=NULL) {
				if (!strcmp(pname[i],predmet[p]->name)) {
					index=p;
					break;
				}
				p++;
	...
	if ((result=mysql_result(query))) {
		while ((row=mysql_fetch_row(result))) {
			...
			for (i=0;i<4;i++)
				convert[total]->component[i]=expand_predmet(row[i]);
			...
			for (i=0;i<4;i++) {
				if (p%10==1)
					convert[total]->eliminate[3-i]=1;
				p/=10;
			}
			for (i=0;i<4;i++) {
				num=0;
				for (p=0;p<4-i;p++)
					if (convert[total]->component[p]>convert[total]->component[num])
						num=p;
...
void do_attack(UR_OBJECT user)
{
	...
	if (user->room==get_room("letun",NULL) || user->room==get_room("plachetnica",NULL))
		return;
	if (tsec%15==0) {
		for (i=0;i<MPVM;++i)
				...
				for (i=0;i<MPVM;++i)
					if (rm->predmet[i]>-1 && predmet[rm->predmet[i]]->type<3 && rm->dur[i]<10000 && rm->dur[i]>=0) {
						if (is_free_in_room(rm)==-1)
							return;
						vec=rm->predmet[i];
						put_in_room(user->room,vec,predmet[vec]->dur);
						rm->predmet[i]=-1;
						rm->dur[i]=0;
						sprintf(text,"~FY%s sa naraz splasil%s a uz%s neni.\n",predmet[vec]->name,zwjpohl(vec,"","a","o",""),zwjpohl(vec," ho"," jej"," ho",""));
						text[3]=toupper(text[3]);
						wrtype=WR_ZVERY;
						write_room(rm,text);
...
void show_pict_on_event(UR_OBJECT user,int event,int vec,int newline)
{
	...
	if (vec==-1)
		return;
	if (predmet[vec]->showpict==0 || predmet[vec]->picture==NULL)
		return;
	if (!(predmet[vec]->showpict & event))
		return;
	if (newline)
		write_user(user,"\n");
	...
	if ((predmet[vec]->showpict & 1)) {
		...
		if (sqlmore(user,user->socket,query)==1) {
			user->misc_op=222;
			return;
		}
		return;
	...
	if (which>0) {
		for (i=ok; i<word_count; i++) {
			...
			if (predmet[vec]->firerate>0 && rand()%100+1<predmet[vec]->firerate) {
				strcat(newimpstr," ");
				strcat(newimpstr,ins[rand()%which]);
			}
...
void do_funct_stuff(UR_OBJECT user,char *inpstr,int start)
{
	...
	if (is_affected(user,1024)>-1)
		pohaluz_predmetom(inpstr,start,is_affected(user,1024));
	else if (is_affected(user,128)>-1)
		zachlastaj2(inpstr,start);
	else if (is_affected(user,256)>-1)
		zalamerizuj(inpstr,start2);
	else if (is_affected(user,512)>-1)
		debilneho(inpstr,start);
	else if (user->zuje)
...
{
	if (user->affected>-1 && predmet[user->affected]->altfunct & flag)
		return user->affected;
	if (user->affpermanent>-1 && predmet[user->affpermanent]->altfunct & flag)
		return user->affpermanent;
	return -1;
}
...
void do_alt_funct(UR_OBJECT user,int vec)
{
	...
	if (
			predmet[vec]->altfunct & 32
			||
			predmet[vec]->altfunct & 128
			||
			predmet[vec]->altfunct & 256
			||
			predmet[vec]->altfunct & 512
			||
			predmet[vec]->altfunct & 1024
	   ) {
		if (defa==-1) {
			user->affpermanent=vec;
			return;
		}
		if (user->affected!=vec)
			user->affecttime=0;
...
void hurt(UR_OBJECT user,UR_OBJECT u,int vec)
{
	...
	for (i=0;i<BODY-1;i++) if (u->predmet[i]>-1) {
		protect=u->predmet[i];
		if (predmet[protect]->defense>-1 && i<HANDS)
			continue;
		if (predmet[protect]->defense==0)
			continue;
	...
	if (u->health<10) {
		if (user!=NULL && before>9) {
			if (user->team!=u->team || u->team==0) {
				...
				if (user->gold/200<1)
					equi=5-5*(user->gold/200);
				if (user->kills>=0)
					user->gold+=10+bonus+equi;
			}
			else {
				user->kills--;
				add_point(user,DB_BRUTALIS,-1,0);
...
int convert_predmet(UR_OBJECT user,int spell)
{
	...
	if (spell==1) {
		...
		for (i=0;i<pocet_konverzii;i++)
			if (!strcmp(convert[i]->spell,word[1]))
				ic=i;
		if (ic==-1)
			return 0;
		for (i=0;i<4;i++) {
	...
	if (spell==0) {
		...
		for (i=0;i<pocet_konverzii;i++) {
			if (convert[i]->spell[0])
				continue;
			if (
					convert[i]->component[0]==pinput[0]
					&&
					convert[i]->component[1]==pinput[1]
					&&
					convert[i]->component[2]==pinput[2]
					&&
					convert[i]->component[3]==pinput[3]
			   ) {
				ic=i;
				break;
			}
	...
	if (convert[ic]->source==0) {
		for (i=4-cnt;i<4;i++) {
			for (p=0;p<HANDS;p++)
				if (user->predmet[p]==pinput[i] && hands[p]==0) {
					foundh[i]=p;
					hands[p]=1;
					if (convert[ic]->eliminate[i])
						hks++;
					ks++;
					break;
				}
		...
		for(i=4-cnt;i<4;i++) {
			for(p=0;p<MPVM;p++)
				if (user->room->predmet[p]==pinput[i] && room[p]==0) {
					foundr[i]=p;
					room[p]=1;
					if (convert[ic]->eliminate[i])
						rks++;
	...
	if (ks!=cnt) {
		if (convert[ic]->missing!=NULL) {
			...
			if (ks==0)
				write_user(user,"Ved nemas ani jeden z predmetov!\n");
			else if (ks==cnt-1)
				write_user(user,"Chyba ti este jeden predmet!\n");
			else if (ks==cnt-2)
				write_user(user,"Chybaju ti este dva predmety!\n");
			else if (ks==cnt-3)
				write_user(user,"Chybaju ti este tri predmety!\n");
		}
		return 1;
	}
	...
	if (spell==1) {
		if (user->mana<convert[ic]->mana) {
			...
			switch (rand()%3) {
				case 0:
					write_user(user,"~FGKuzlo sa takmer podarilo, no zlyhalo na nedostatku tvojej magickej energie...\n");
					break;
				case 1:
					write_user(user,"~FGKuzlo bolo spravne, no tvoja sila je primala...\n");
					break;
				case 2:
					write_user(user,"~FGVzduch sa zachvel, no tvoje sily na dokoncenie kuzla nestacili...\n");
					break;
			}
			return 1;
		}
		else
			user->mana-=convert[ic]->mana;
	}
	...
	for (i=4-cnt;i<4;i++)
		if (convert[ic]->eliminate[i]) {
			if (foundh[i]>-1)
				user->predmet[foundh[i]]=-1;
			else
				if (foundr[i]>-1)
					user->room->predmet[foundr[i]]=-1;
		}
...
void use_predmet(UR_OBJECT user,char *inpstr)
{
	...
	if (
			...
			&&
			(predmet[vec]->function!=21)
			&&
			(predmet[vec]->function!=22)
	   ) {
		if (where>-1 && predmet[vec]->function==5) {
			for (i=0;i<MPVM;i++)
				if (user->room->predmet[i]==vec && user->room->dur[i]%10000==user->socket)
					where=i;
		...
		switch (predmet[vec]->function) {
			case 14:
				if ((user->level<MOZENADAVAT) && (contains_swearing(inpstr,user)) && (ban_swearing)) {
					...
					sprintf(text,"~OL~FR\07[CENZURA: %s sa pokusil%s skaredo telefonovat!]~RS\n",user->name,pohl(user,"","a"));
				...
				for (u=user_first;u!=NULL;u=u->next) {
					if (
							...
							u->ignfun
							||
							u->room->group!=user->room->group
							||
							u->igntell
							||
							check_ignore_user(user,u)
							||
							u==user
							||
							u->type!=USER_TYPE
		...
		if (((u==NULL && vec2>-1)) || ((u!=NULL) && (kder>-1 || kdeh>-1))) {
			if (kder==-1 && kdeh==-1) {
				if (predmet[vec2]->type<3)
					...
					if (rand()%3==0)
						sprintf(text,"Hmm.. tak takto to asi nepojde.\n");
					else
						sprintf(text,"Takto %s nemozes pouzit.\n",predmet[vec]->akuzativ);
				}
				write_user(user,text);
				return;
			}
			...
			if (u==user)
				...
				if (u->ignall) {
					if (u->filepos)
						sprintf(text,"%s prave cita nejaky text a nechce byt rusen%s.\n",u->name,pohl(u,"y","a"));
					else if (u->malloc_start!=NULL)
						sprintf(text,"%s prave edituje nejaky text, a nechce byt rusen%s.\n",u->name,pohl(u,"y","a"));
					else
						sprintf(text,"%s teraz vsetkych ignoruje.\n",u->name);
					write_user(user,text);
					return;
				}
				switch (predmet[vec]->function) {
					...
					case 3:
						...
						if (nah==2) {
							...
							if (!strcmp(predmet[vec]->name,"zuby"))
								strcat(text,"Len-len sa ti podarilo vyslobodit si hlavu ;>>");
							strcat(text,"\n");
						...
						return;
					case 8:
						if (forbidden(user,u,vec))
							return;
						show_pict_on_event(user,32,vec,0);
						show_pict_on_event(u,32,vec,1);
						nah=0;
						if (rand()%100+1<predmet[vec]->seconddur)
							nah=1;
						remove_from_hands(user,vec,1);
						if (predmet[vec]->userphr!=NULL) {
		...
		switch (predmet[vec]->function) {
			...
			case 2:
				...
				if (dur>1) {
					...
					sprintf(text,"Vsetci okolostojaci strpli od napatia, ked tvoj prst siahol po spusti a\npotiahol ju ... CLICK! ... tentokrat si mal%s stastie!~LB\n",pohl(user,"","a"));
					write_user(user,text);
					sprintf(text,"~FG%s si prilozil%s %s k spanku a stlacil%s spust ... CLICK!\n~FGTo ma niekto stastie ...\n",name,pohl(user,"","a"),predmet[vec]->akuzativ,pohl(user,"","a"));
					write_room_except(user->room,text,user);
					do_alt_funct(user,vec);
					return;
				}
				user->dur[is_in_hands(user,vec)]=(rand()%6)+1;
				sprintf(text,"~LB~OL~LIBANG!!! ~RS~FRMiestnostou sa ozval tupy vystrel a v bolestiach si sa zrutil%s na zem.\n",pohl(user,"","a"));
				write_user(user,text);
				...
				if (predmet[vec]->food[0]) {
					...
					if (vec2>-1) {
						if (predmet[vec2]->function==24) {
							...
							for (i=0;i<MPVM;i++)
								if (user->room->predmet[i]==vec2 && user->room->dur[i]>9999 && user->room->dur[i]%10000==user->socket) {
									kdeh=i;
									nah=user->room->dur[i]/10000;
									break;
								}
							if (kdeh==-1)
								kdeh=kder;
...
void throw_predmet(UR_OBJECT user)
{
	...
	if (word_count==3) {
		...
		u=NULL;
		if (user->room->group==4 || user->level==GOD || predmet[vec]->altfunct & 2)
			u=get_user(word[2]);
		if (rm==NULL && u==NULL) {
		...
		if (ok==0) {
			if (u!=NULL) {
				...
				if (predmet[vec]->altfunct & 2) {
					if (dur>1)
						put_in_room(u->room,vec,dur-1);
				}
				else
					put_in_room(u->room,vec,dur);
				return;
			}
	...
	if (user->room->group==4 || user->level==GOD)
		write_user(user,"Co chces kam hodit? Pouzi: .throw <vec> <miestnost> alebo: .throw <vec> <user>\n");
	else
		write_user(user,"Co chces kam hodit? Pouzi: .throw <vec> <miestnost>\n");
}
...
void dispose_predmet(UR_OBJECT user)
{
	...
	if (user->level==GOD && (!strcmp(word[1],"everyroom") || !strcmp(word[1],"everything"))) {
		...
		if (!strcmp(word[1],"everyroom")) {
			sprintf(text,"Bol%s zlikvidovan%s %d predmet%s.\n",skloncislo(vec,"","i","o"),skloncislo(vec,"y","e","ych"),vec,skloncislo(vec,"","y","ov"));
			write_user(user,text);
			return;
		}
	}
	...
	sprintf(text,"Zrusil%s si %s %s.\n",pohl(user,"","a"),predmet[vec]->akuzativ,user->room->from);
	write_user(user,text);
	sprintf(text,"~OL~FBBoooo! ~RS~FR%s vyriekol zaklinadlo a %s sa straca v hustej hmle!\n",name,predmet[vec]->name);
	if (user->vis && strcmp(word[word_count-1],"quiet"))
		write_room_except(user->room,text,user);
}
...
void give_predmet(UR_OBJECT user)
{
	...
	if (u->ignall) {
		if (u->filepos)
			...
			write_user(user,"Uzivatel prave edituje nejaky text, a nechce byt ruseny.\n");
		else
			write_user(user,"Uzivatel teraz vsetkych ignoruje.\n");
		return;
	}
	dur=user->dur[is_in_hands(user,vec)];
	if ((predmet[vec]->function==12) && (dur)) {
		...
		if (u->ignfun) {
			write_user(user,"Ziadne bombove srandicky, uzivatel ignoruje vsetku zabavu :-(\n");
			return;
		}
		...
		sprintf(text,"%s ti podal%s tikajuce cudo (vyzera to ako bomba :-)!!!\n",user->name,pohl(user,"","a"));
		write_user(u,text);
		sprintf(text,"%s sa zbavil%s tikajucej bomby, ktoru podal%s %s!\n",user->name,pohl(user,"","a"),pohl(user,"","a"),sklonuj(u,3));
	...
	if (!u->vis)
		strcpy(uname,pohl(u,"Neznamemu","Neznamej"));
	else
		strcpy(uname,sklonuj(u,3));
...
void create_predmet(UR_OBJECT user)
{
	...
	if (word_count<2) {
		if (user->level==GOD)
			write_user(user,"Pouzi: .create <predmet>  alebo: .create roomlink <room1> <room2>\n");
		else
			write_user(user,"Pouzi: .create <predmet>\n");
	...
	if (word_count>2 && (u=get_user(word[2])) && (predmet[vec]->type<3)) {
		if (predmet[vec]->function==24)
			dur=u->socket+predmet[vec]->dur*10000;
		else
			dur=u->socket+10000;
	}
	put_in_room(user->room,vec,dur);
	...
	write_user(user,text);
	sprintf(text,"~OL~FRHops! ~RS~FG%s vyriek%s zaklinadlo a vytvoril%s ~OL%s~RS~FG!\n",name,pohl(user,"ol","la"),pohl(user,"","a"),predmet[vec]->akuzativ);
...
void get_predmet(UR_OBJECT user)
{
	...
	if (word_count<2) {
		...
		for (i=0;i<MPVM;++i) {
			if (user->room->predmet[i]>-1) {
				...
				if (predmet[user->room->predmet[i]]->type==3 || predmet[user->room->predmet[i]]->type==4)
					strcat(text,"~FW");
				else
					strcat(text,farba_p(user->room->predmet[i]));
	...
	if (word_count==3) {
		...
		if (idle<=10) {
			sprintf(text,"%s je cul%s ako vevericka, nemozes %s odcudzit %s.\n",u->name,pohl(u,"y","a"),pohl(u,"mu","jej"),predmet[vec]->akuzativ);
			write_user(user,text);
			return;
		}
		if (user->pp<STEALCAN) {
...
void put_predmet(UR_OBJECT user)
{
	...
	if (!strcmp(word[1],"all")) {
		...
		for (i=0;i<HANDS;i++)
			if (user->predmet[i]==12 && user->dur[i]>0) {
		...
		if (user->vis)
			...
			name=invisname2(user);
		sprintf(text,"Polozil%s si %s",pohl(user,"","a"),text2);
		write_user(user,text);
		sprintf(text,"%s polozil%s %s",name,pohl(user,"","a"),text2);
		write_room_except(user->room,text,user);
		user->sell_what=-1;
		user->sell_price=0;
		user->sell_to[0]='\0';
...
void buy(UR_OBJECT user)
{
	...
	if (!strcmp(user->room->name,"obchod")) {
		if (word_count==1) {
			for (i=0;predmet[i]!=NULL;i++)
				if (predmet[i]->price>0) {
					cnt++;
					if (cnt==1) {
						sprintf(text,"~OLV obchode ponukaju:~RS ");
						vel=strlen(text)-6;
					}
			...
			if (vec>-1) {
				...
				if (user->gold<predmet[vec]->price) {
					sprintf(text,"%s stoji %d dukat%s, tolko nemas.\n",predmet[vec]->name,predmet[vec]->price,skloncislo(predmet[vec]->price,"","y","ov"));
					text[0]=toupper(text[0]);
					write_user(user,text);
					return;
				}
					...
					if (predmet[vec]->function==24) {
						if (is_free_in_room(user->room)==-1)
							user->room->predmet[rand()%MPVM]=-1;
						put_in_room(user->room,vec,predmet[vec]->dur*10000+user->socket);
					}
					else
						put_in_hands(user,vec,default_dur(vec));
					user->gold-=predmet[vec]->price;
					sprintf(text,"~FYKupil si ~OL%s~RS~FY za ~OL%d~RS~FY dukat%s.",predmet[vec]->akuzativ,predmet[vec]->price,skloncislo(predmet[vec]->price,"","y","ov"));
					word_count=1;
					gold(user,1,text);
					return;
				}
		...
		for (u=user_first;u!=NULL;u=u->next)
			if (
					...
					(
					 u->sell_to[0]=='\0'
					 ||
					 !strcmp(u->sell_to,user->name)
					)
			   ) {
				if (u->sell_to[0]=='\0')
					sprintf(texthb,"%s tu ponuka %s za %d dukat%s.\n",u->name,predmet[u->sell_what]->akuzativ,u->sell_price,skloncislo(u->sell_price,"","y","ov"));
				else
					sprintf(texthb,"~OL%s~RS ti ponuka %s za %d dukat%s.\n",u->name,predmet[u->sell_what]->akuzativ,u->sell_price,skloncislo(u->sell_price,"","y","ov"));
				strcat(text,texthb);
			}
		write_user(user,text);
		return;
	...
	if (((u->sell_to[0]) && strcmp(user->name,u->sell_to)) || u->sell_price==0 || u->sell_what==-1) {
	...
	if (u->sell_price>user->gold) {
		sprintf(text,"%s chce za %s %d dukat%s, tolko nemas.\n",u->name,predmet[vec]->akuzativ,u->sell_price,skloncislo(u->sell_price,"","y","ov"));
		write_user(user,text);
		return;
	}
	if (is_free_in_hands(user)==-1) {
	...
	if (user->vis)
		...
		name=invisname(user);
	sprintf(text,"~OL%s~RS~FY od teba kupil%s ~OL%s~RS~FY za ~OL%d~RS~FY dukat%s.",name,pohl(user,"","a"),predmet[vec]->akuzativ,u->sell_price,skloncislo(u->sell_price,"","y","ov"));
	gold(u,2,text);
	sprintf(text,"Kupil%s si od ~OL%s %s~RS~FY za ~OL%d~RS~FY dukat%s.",pohl(user,"","a"),sklonuj(u,2),predmet[vec]->akuzativ,u->sell_price,skloncislo(u->sell_price,"","y","ov"));
	gold(user,1,text);
	u->sell_to[0]='\0';
	u->sell_what=-1;
	u->sell_price=0;
...
void sell(UR_OBJECT user)
{
	...
	if (word_count<3) {
		...
		if (user->sell_price==0) {
			...
			if (user->sell_to[0]) {
				...
				sprintf(text,"Prave ponukas ~OL%s %s~RS za ~OL%d~RS dukat%s.\n",sklonuj(u,3),predmet[user->sell_what]->akuzativ,user->sell_price,skloncislo(user->sell_price,"","y","ov"));
				write_user(user,text);
				return;
			}
			sprintf(text,"Prave ponukas ~OL%s~RS za ~OL%d~RS dukat%s.\n",predmet[user->sell_what]->akuzativ,user->sell_price,skloncislo(user->sell_price,"","y","ov"));
			write_user(user,text);
			return;
		}
	}
	...
	if (word_count>3) {
		...
		if (!strcmp(user->sell_to,u->name) && user->sell_what==vec && user->sell_price==offer) {
		...
		sprintf(text,"~OL%s~RS ti ponuka ~OL%s~RS za ~OL%d~RS dukat%s.\n",name,predmet[vec]->akuzativ,offer,skloncislo(offer,"","y","ov"));
		write_user(u,text);
		sprintf(text,"Ponukas ~OL%s %s~RS za ~OL%d~RS dukat%s.\n",sklonuj(u,3),predmet[vec]->akuzativ,offer,skloncislo(offer,"","y","ov"));
		write_user(user,text);
		return;
	}
	...
	user->sell_price=offer;
	user->sell_to[0]='\0';
	sprintf(text,"Ponukas ~OL%s~RS za ~OL%d~RS dukat%s.\n",predmet[vec]->akuzativ,offer,skloncislo(offer,"","y","ov"));
	write_user(user,text);
	sprintf(text,"~OL%s~RS ponuka ~OL%s~RS za ~OL%d~RS dukat%s.\n",name,predmet[vec]->akuzativ,offer,skloncislo(offer,"","y","ov"));
	write_room_except(user->room,text,user);
}
...
void gold(UR_OBJECT user,int left,char *str)
{
	...
	if (word_count==1) {
		...
		if (user->gold==0) {
			if (left==1)
				sprintf(text,"~FY  / ~OL$~RS~FY \\  V mesci ti nic nezostalo.\n");
			else
				sprintf(text,"~FY  / ~OL$~RS~FY \\  Tvoj mesec je prazdny.\n");
		}
		else {
			if (left==1)
				sprintf(text,"~FY  / ~OL$~RS~FY \\  V mesci ti ostal%s ~OL%d~RS~FY dukat%s~FY.\n",skloncislo(user->gold,"","i","o"),user->gold,skloncislo(user->gold,"","y","ov"));
			else
				sprintf(text,"~FY  / ~OL$~RS~FY \\  Momentalne mas v mesci ~OL%d~RS~FY dukat%s~FY.\n",user->gold,skloncislo(user->gold,"","y","ov"));
		}
		write_user(user,text);
...
int remove_from_room(RM_OBJECT rm,int vec,int dur)
{
	...
	if (pos==-1)
		return -1;
	if ((predmet[vec]->altfunct & 1) && rm->dur[pos]>dur)
		rm->dur[pos]-=dur;
	else {
		rm->predmet[pos]=-1;
		rm->dur[pos]=0;
...
int is_in_room(RM_OBJECT rm,int vec)
{
	...
	if (retval>-1 && rm->dur[retval]>9999)
		for (i=retval+1;i<MPVM;++i)
			if (rm->predmet[i]==vec && rm->dur[i]<10000) {
				retval=i;
				break;
			}
	return retval;
...
int is_funct_in_room(RM_OBJECT rm,int fun)
{
	...
	for (i=0; i<MPVM; ++i)
		if (rm->predmet[i]>-1 && (predmet[rm->predmet[i]]->altfunct & fun))
			return i;
	return -1;
}
...
int is_funct_in_hand(UR_OBJECT u,int fun)
{
	...
	for (i=0; i<HANDS; ++i)
		if (u->predmet[i]>-1 && (predmet[u->predmet[i]]->altfunct & fun))
			return i;
	return -1;
}
...
int spravny_predmet(char *vec1)
{
	...
	i=0;
	while (predmet[i]!=NULL) {
		if (!strcasecmp(predmet[i]->name,vec))
			return 1;
		++i;
	}
	return 0;
...
int expand_predmet(char *vec1)
{
	...
	if (!vec1[0])
		return -1;
	while (predmet[i]!=NULL) {
		if (!strncasecmp(predmet[i]->name,vec1,strlen(vec1)))
			return i;
		++i;
	}
	return -1;
...
int default_dur(int vec)
{
	...
	if (vec==-1 || predmet[vec]==NULL)
		return 0;
	if (predmet[vec]->function==15)
		return 0;
	if (predmet[vec]->type<3) {
...
int pohaluz(UR_OBJECT user)
{
	...
	c+=((((g+42)*8)%9)*3)%9-22;
	for(i=0;i<l;i++)
		c+=(((n[i])*42)%8)*(8+i*9+i%3+i/4)+(((g+666)%42)%8)*9;
	for(i=0;i<l;i++)
		c+=((n[i]*88)%9)*((i/3+93)+((i*7)+(((i%4)))));
	c+=(((user->gold+999)/42)%8)*888;
	return c;
	/***
	 * Original code of the above calculations looked like this ;)
	 *
	 * c=     0;  c+=  ((((g+   42)*8)  %9)*3)
	 * %9-   22;  for  (i=  0;  i<l     ;i  ++)
	 *  c+= (((   n[i  ])*42)   %8)*(   8+i*9+
...
void hesh(RM_OBJECT rm)
{
	...
	for (i=0;i<MPVM;i++)
		if (rm->predmet[i]>-1 && predmet[rm->predmet[i]]->type<3 && predmet[rm->predmet[i]]->function!=24 && rm->dur[i]<10000)
			rm->dur[i]=0;
	do_predmety(1);
}
				...
				if (predmet[user->predmet[i]]->type<10) {
					...
					if (user->dur[i]<1 || (user->dur[i]>9999 && user->socket!=(user->dur[i]%10000) && tsec%10==0)) {
						...
						if (predmet[user->predmet[i]]->type<3) {
							...
							if (predmet[user->predmet[i]]->altfunct & 2048)
								user->lsd+=90;
							if (is_free_in_room(user->room)>-1) {
								if (user->vis)
									strcpy(name,sklonuj(user,3));
								else
									strcpy(name,pohl(user,"neznamemu","neznamej"));
	...
	for (rm=room_first;rm!=NULL;rm=rm->next)
		for (i=0;i<MPVM;i++)
			if (rm->predmet[i]>-1 && predmet[rm->predmet[i]]->function!=24) {
								...
								if (u->room->group!=rm->group)
									...
									if (is_free_in_room(u->room)>-1 && (int)(rand()%3)==0) {
										...
										if (predmet[rm->predmet[i]]->searchphr!=NULL) {
											...
											if (u->vis)
												strcpy(tmp,sklonuj(u,4));
											else
												strcpy(tmp,pohl(u,"Neznameho","Neznamu"));
										...
										nah=rand()%3;
										if (u->vis)
											strcpy(tmp,sklonuj(u,3));
										else
											strcpy(tmp,pohl(u,"Neznamemu","Neznamej"));
				...
				if (user_trigged==0 && (predmet[rm->predmet[i]]->type==3 || predmet[rm->predmet[i]]->type==4) && rm->dur[i]<predmet[rm->predmet[i]]->dur) {
					rm->dur[i]-=1;
					if (rm->dur[i]<1) {
					...
					if (rm->dur[i]>0) {
						...
						if (rm->dur[i]==0) {
							if (predmet[rm->predmet[i]]->victimphr!=NULL) {
								for(u=user_first;u!=NULL;u=u->next) {
									if (u->type==CLONE_TYPE || !u->zaradeny || u->login || u->room!=rm)
										continue;
									newr=random_room(rm,0);
									if (newr!=NULL) {
										tmp[0]='\0';
...
char *zwjpohl(int vec,char *muz,char *zena,char *ono,char *ich)
{
	if (predmet[vec]==NULL)
		return (muz);
	if (predmet[vec]->type==2 || predmet[vec]->type==12)
		return (ono);
	else if (predmet[vec]->type==1 || predmet[vec]->type==11 || predmet[vec]->type==4)
		return(zena);
	else if (predmet[vec]->type==13)
		return(ich);
	return (muz);
}
...
char *parse_phrase(char *str,UR_OBJECT user,UR_OBJECT u,RM_OBJECT rm,int whichtype)
{
	...
	for (i=0;i<word_count;i++) {
		if (i>0)
			strcat(ret," ");
		if (strchr(word[i],'$') || strchr(word[i],'\\') || strchr(word[i],'_') || strchr(word[i],'%')) {
			cnt=0;
			where=0;
			tmp[0]='\0';
				...
				if (cnt==0 || cnt==sex) {
					if (word[i][pos]=='%') {
						...
						if (user!=NULL) {
							if (word[i][pos]=='1') {
								if (user->vis)
									strcat(tmp2,user->name);
								else
									strcat(tmp2,pohl(user,"Neznamy","Neznama"));
							}
							if (word[i][pos]=='2') {
								if (user->vis)
									strcat(tmp2,sklonuj(user,2));
								else
									strcat(tmp2,pohl(user,"Neznameho","Neznamej"));
							}
							if (word[i][pos]=='3') {
								if (user->vis)
									strcat(tmp2,sklonuj(user,3));
								else
									strcat(tmp2,pohl(user,"Neznamemu","Neznamej"));
							}
							if (word[i][pos]=='4') {
								if (user->vis)
									strcat(tmp2,sklonuj(user,4));
								else
									strcat(tmp2,pohl(user,"Neznameho","Neznamu")); }
							if (word[i][pos]=='w')
								strcat(tmp2,user->room->where);
							if (word[i][pos]=='i')
								strcat(tmp2,user->room->into);
							if (word[i][pos]=='f')
								strcat(tmp2,user->room->from);
							if (word[i][pos]=='t')
								strcat(tmp2,teamcolor[user->team]);
						}
						if (u!=NULL) {
							if (word[i][pos]=='5') {
								if (u->vis)
									...
									strcat(tmp2,pohl(u,"Neznamy","Neznama"));
							}
							if (word[i][pos]=='6') {
								if (u->vis)
									strcat(tmp2,sklonuj(u,2));
								else
									strcat(tmp2,pohl(u,"Neznameho","Neznamej"));
							}
							if (word[i][pos]=='7') {
								if (u->vis)
									strcat(tmp2,sklonuj(u,3));
								else
									strcat(tmp2,pohl(u,"Neznamemu","Neznamej"));
							}
							if (word[i][pos]=='8') {
								if (u->vis)
									strcat(tmp2,sklonuj(u,4));
								else
									strcat(tmp2,pohl(u,"Neznameho","Neznamu")); }
							if (word[i][pos]=='T')
								strcat(tmp2,teamcolor[u->team]);
						}
						if (sendvec>-1 && u==NULL) {
							if (word[i][pos]=='5')
								strcat(tmp2,predmet[sendvec]->name);
							if (word[i][pos]=='6')
								strcat(tmp2,predmet[sendvec]->inytiv);
							if (word[i][pos]=='7')
								strcat(tmp2,predmet[sendvec]->dativ);
							if (word[i][pos]=='8')
								strcat(tmp2,predmet[sendvec]->akuzativ);
						}
						if (rm!=NULL) {
							if (word[i][pos]=='W')
								strcpy(tmp2,rm->where);
							if (word[i][pos]=='I') {
								if (rm->group==4)
									strcpy(tmp2,"smerom ");
								strcat(tmp2,rm->into);
					...
					else if (word[i][pos]=='_') {
						tmp[where]=' ';
						where++;
					}
					else {
						tmp[where]=word[i][pos];
						where++;
					}
				}
			}
