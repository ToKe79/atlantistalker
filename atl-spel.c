...
extern void show_kniha_kuziel(UR_OBJECT user,int dur)
{
	...
	sprintf(filename,"%s%ckuzlo.%02d",ITEMFILES,DIRSEP,rand()%(POCET_KUZIEL+4));
...
int mana_check(UR_OBJECT user, int howmuch, int first, int second)
{
	...
	if (first!=-1 && second!=-1) {
		ok=0;
		for (i=0;i<HANDS;i++) {
			if (user->predmet[i]==first) {
				ok++;
				continue;
			}
			if (user->predmet[i]==second)
				ok++;
		}
		if (ok<2) {
	...
	if (rand()%prob) {
		switch(rand()%5) {
			case 0:
				write_user(user,"~FGHoci formulka bola spravna, kuzlo zlyhalo... Zeby chybali skusenosti?\n");
				break;
			case 1:
				write_user(user,"~FGVzduch sa zviril, kuzlo sa zdvihlo... no zoslablo a padlo...\n");
				break;
			case 2:
				write_user(user,"~FGKuzlo sa roztocilo, vzlietlo... no zrutilo sa nazad na zem...\n");
				break;
			case 3:
				write_user(user,"~FGMoc kuzla bola zahadne oslabena a kuzlo sa rozpadlo na criepky...\n");
				break;
			case 4:
				write_user(user,"~FGKuzlo vzlietlo, no premenilo sa na dym a rozplynulo sa...\n");
				break;
		}
		user->mana-=((rand()%howmuch)/2);
		return 1;
	...
	if (first!=-1)
		remove_from_hands(user,first,0);
	if (second!=-1)
		remove_from_hands(user,second,0);
	return 0;
}
...
{
	char *runes[]={"ar","bro","fur","grd","ku","jag","lo","me","nop","paw","sup","tra","vis","woch","yo","*"};
	char *abc[]={"a","b","f","g","k","j","l","m","n","p","s","t","v","w","y"};
	char *allowcom[]={".kiss",".go",".hug",".shout",".tell","*"};
	/* v allowcom su prikazy, ktore je mozno forcovat cez kuzla ;-) */
	UR_OBJECT target=user;
	RM_OBJECT room;
	char mword[1024],*ptr,*name;
	unsigned int i;
	...
	switch (mword[1]){
		...
		default:
			switch(rand()%5) {
				case 0:
					write_user(user,"~FGOpakujes si runovu abecedu?\n");
					break;
				case 1:
					write_user(user,"~FGTaketo kuzlo nema hlavu ani patu...\n");
					break;
				case 2:
					write_user(user,"~FGNechces si najskor nastudovat kuzelnicku gramatiku?\n");
					break;
				case 3:
					write_user(user,"~FGSlova, iba slova... tie kuzla nestvoria!\n");
					break;
				case 4:
					write_user(user,"~FGKuzlo potrebuje svoj ciel a vediet, co ma robit...\n");
					break;
			}
			return;
	}
	...
	switch (mword[0]) { /* hlavny vypinac (switch) */
		case 'b': /* vytvorenie */
			...
			if (!strcmp(ptr,"p")) { /* pridavanie PP */
				if (mana_check(user,20,expand_predmet("skorocel"),-1))
					return;
				...
				if (mana_check(user,50,expand_predmet("pupava"),expand_predmet("pupava")))
					return;
				...
				if (mana_check(user,15,expand_predmet("cernobyl"),expand_predmet("skorocel")))
					return;
				...
				if (mana_check(user,80,expand_predmet("pupava"),expand_predmet("skorocel")))
					return;
				if (word_count<3) {
		...
		case 'k': /* likvidacia */
			if (target!=NULL && !strcmp(ptr,"p")) { /* uberanie PP */
				if (mana_check(user,50,expand_predmet("cernobyl"),-1))
					return;
				...
				if (mana_check(user,90,expand_predmet("cernobyl"),expand_predmet("cernobyl")))
					return;
				...
				sprintf(text,"~FRspalen%s magickym plamenom",pohl(target,"y","a"));
				...
				if (mana_check(user,90,expand_predmet("cernobyl"),expand_predmet("pupava")))
					return;
				...
				sprintf(text,"~FRspalen%s magickym bleskom",pohl(target,"y","a"));
				logout_user(target,text);
				if (user!=target)
					inyjefuc=1;
				return;
				...
				if (mana_check(user,70,expand_predmet("cernobyl"),expand_predmet("pupava")))
					return;
				if (!flyer.pozicia) {
		...
		case 's': /* premena */
			if (target!=NULL && !strcmp(ptr,"vnv")) { /* neviditelnost */
				if (mana_check(user,50,expand_predmet("invisibylina"),-1))
					return;
				...
				if (mana_check(user,10,expand_predmet("invisibylina"),-1))
					return;
				...
				if (mana_check(user,20,expand_predmet("zaba"),-1))
					return;
				...
				if (mana_check(user,20,expand_predmet("sliepka"),-1))
					return;
				...
				if (mana_check(user,30,expand_predmet("invisibylina"),expand_predmet("skorocel")))
					return;
				if (target->level>=KIN) {
				...
				if (target->cloak) {
					...
					write_user(user,"~FGKuzlo odkrytia levelu nenaslo level zakryty...\n");
				return;
			}
			break;
		case 't': /* presun */
			if (target!=NULL && !strcmp(ptr,"gwa")) { /* presun kamsi odveci */
				if (mana_check(user,50,expand_predmet("skorocel"),expand_predmet("skorocel")))
					return;
				...
				if (mana_check(user,70,expand_predmet("invisibylina"),expand_predmet("invisibylina")))
					return;
	...
	switch(rand()%5) {
		case 0:
			write_user(user,"~FGNepodarilo sa ti vykuzlit nic...\n");
			break;
		case 1:
			write_user(user,"~FGKuzlo nema nijaky zmysel...\n");
			break;
		case 2:
			write_user(user,"~FGPar run pokope kuzlo netvori...\n");
			break;
		case 3:
			write_user(user,"~FGSnaha sa ceni, no znalosti ti chybaju...\n");
			break;
		case 4:
			write_user(user,"~FGZ tohoto sa veru kuzlo nezrodi...\n");
			break;
	}
}

XA_OBJECT create_xannel(void)
{
	XA_OBJECT xannel;
	int i;
	...
	for (i=0;i<REVIEW_LINES;i++)
		xannel->buffer[i]=NULL;
	xannel->creator=NULL;
	xannel->topic[0]='\0';
	return xannel;
}

void destruct_xannel(UR_OBJECT user)
{
	free(user->xannel);
}
...
void xsay(UR_OBJECT user,char *inpstr)
{
	...
	if ((user->level<MOZENADAVAT) && (contains_swearing(inpstr,user)) && (ban_swearing)) {
		...
		sprintf(text,"~OL~FR\07[CENZURA: %s sa pokusil%s kanalovat oplzle vyrazy!]~RS\n",user->name, pohl(user,"","a"));
	...
	if (cnt>1) {
		...
		sprintf(text,"~FG\252C5Ak sa chcete rozpravat dvaja, vyskusajte prikaz tell.\n");
	else
		sprintf(text,"~FG\252C5Samomluva je 666-tym priznakom sialenstva.\n");
	write_user(user,text);
}

void record_xannel(XA_OBJECT xannel,char *str)
{
	wash_bell(str);
	if (xannel->buffer[xannel->line]!=NULL)
		free((void *)xannel->buffer[xannel->line]);
...
void add2xannel(UR_OBJECT user,UR_OBJECT u)
{
	...
	if (user->xannel->type==2)
		strcat(text,"privatneho");
	else if (user->xannel->type==1)
		strcat(text,"zavreteho");
	else
		strcat(text,"verejneho");
...
void xannel(UR_OBJECT user,char *inpstr)
{
	...
	if (!strcmp(word[1],"open") || !strcmp(word[1],"public") || !strcmp(word[1],"private")) {
		...
		if (type==2)
			sprintf(text,"~FG\252C5Otvoril%s si si ~FWprivatny~FG\252C5 kanal na temu: ~OL~FW%s\n",pohl(user,"","a"),inpstr);
		else if (type==1)
			sprintf(text,"~FG\252C5Otvoril%s si ~FWuzavrety~FG\252C5 kanal na temu: ~OL~FW%s\n",pohl(user,"","a"),inpstr);
		else
			sprintf(text,"~FG\252C5Otvoril%s si ~FWverejny~FG\252C5 kanal na temu: ~OL~FW%s\n",pohl(user,"","a"),inpstr);
		write_user(user,text);
		return;
	}
	if (!strcmp(word[1],"who")) {
		...
		if (user->xannel->type==2)
			sprintf(text,"~FG\252C5Momentalne sa nachadzas v ~FWprivatnom~FG\252C5 kanali.\n");
		else if (user->xannel->type==1)
			sprintf(text,"~FG\252C5Momentalne sa nachadzas v ~FWzavretom~FG\252C5 kanali.\n");
		else
			sprintf(text,"~FG\252C5Momentalne sa nachadzas vo ~FWverejnom~FG\252C5 kanali.\n");
		write_user(user,text);
	...
	if (!strcmp(word[1],"all")) {
		...
		for (u=user_first;u!=NULL;u=u->next)
			if (u->xannel!=NULL && u->xannel->creator==u) {
				total++;
				if (total==1)
					write_user(user,title("~FG\252C5Zoznam kanalov","~FG"));
				if (u->xannel->type==2)
					sprintf(text,"~FG\252C5Privatny:~FW~OL %s \n ",u->xannel->topic);
				else if (u->xannel->type==1)
					sprintf(text,"~FG\252C5Uzavrety:~FW~OL %s\n ",u->xannel->topic);
				else
					sprintf(text,"~FG\252C5Verejny:~FW~OL %s\n ",u->xannel->topic);
				len=1;
				cnt=0;
				uu=u;
				while (cnt==0 || uu!=u) {
		...
		if (total)
			...
			write_user(user,"~FG\252C5Nie su otvorene ziadne kanaly.\n");
		return;
	}
	if (!strcmp(word[1],"rev") || !strcmp(word[1],"review")) {
		...
		if (word_count==2) {
			...
			kolko=REVIEW_LINES;
		for(i=0;i<REVIEW_LINES;++i)
			if (user->xannel->buffer[(user->xannel->line+i)%REVIEW_LINES]!=NULL)
				total++;
		cnt=0;
		strtolower(word[2]);
		strtolower(word[3]);
		for(i=0;i<REVIEW_LINES;++i) {
			...
			if (user->xannel->buffer[line]!=NULL) {
				cnt++;
				if (cnt==1)
					write_user(user,title("~FG\252C5Posledne hlasky v kanali","~FG"));
				if (total-kolko<cnt) {
					pom=(char *) malloc ((strlen(user->xannel->buffer[line])*sizeof(char))+1);
					strcpy(pom, user->xannel->buffer[line]);
					strtolower(pom);
					if (word_count>1 && strlen(word[2])>1 && !strstr(pom,word[2]))
						continue;
					if (word_count==3 && !strstr(pom,word[3]))
						continue;
		...
		if (!cnt)
			write_user(user,"~FG\252C5V kanali sa este nehovorilo.\n");
		else
			write_user(user,"~FG.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-\n");
		return;
	}
	if (!strcmp(word[1],"leave") || !strcmp(word[1],"stop") || !strcmp(word[1],"quit") || !strcmp(word[1],"close")) {
		if (user->xannel==NULL) {
			write_user(user,"~FG\252C5Nie si v ziadnom kanali.\n");
			return;
		}
		...
		user->nox=NULL;
		user->xannel=NULL;
		sprintf(text,"~FG\252C5Odis%s si z kanala.\n",pohl(user,"iel","la"));
		write_user(user,text);
		return;
	}
	if (!strcmp(word[1],"kick")) {
		...
		sprintf(text,"\252C5~FW- %s~FG\252C5 bol%s odtialto vystrnaden%s.\n",uu->name,pohl(uu,"","a"),pohl(uu,"y","a"));
		cnt=0;
		u=uu;
		while (u->nox!=uu) {
			cnt++;
	...
	if (word_count==2) {
		...
		if (user->xannel!=NULL) {
			if (user->xannel->type==2 && user->xannel->creator!=user) {
				sprintf(text,"~FG\252C5Tento kanal otvoril%s ~FW%s~FG\252C5, len on%s moze pozyvat.\n",pohl(user->xannel->creator,"","a"),user->xannel->creator->name,pohl(user->xannel->creator,"","a"));
				write_user(user,text);
				return;
			}
			if (user->xannel->type==2) {
			...
			if (u->xannel->type==2 && u!=u->xannel->creator) {
				sprintf(text,"\252C5~FW%s~FG\252C5 je v privatnom kanali, ktory otvoril%s ~FW%s~FG\252C5, opytaj sa je%s.\n",u->name,pohl(u->xannel->creator,"","a"),u->xannel->creator->name,pohl(u->xannel->creator,"ho","j"));
				write_user(user,text);
				return;
			}
			if (u->invite2xannel==user) {
				...
				sprintf(text,"~FG\252C5Pytas sa ~FW%s~FG\252C5 ci mozes ist za n%s do kanala.\n",sklonuj(u,2),pohl(u,"im","ou"));
				write_user(user,text);
