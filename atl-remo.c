	...
	for (user=user_first;user!=NULL;user=user->next) {
		...
		if (waitpid (user->lynx, &status, WNOHANG) == user->lynx) {
			...
			switch (more(user,user->socket,filename)) {
				case 0:
					write_user(user,"~FTLynx~FW: Subor nenajdeny!\n");
					break;
				case 1:
					user->misc_op=2;
					break;
...
char *real_auth(UR_OBJECT gdo)
{
	...
	register unsigned long inlocal,inremote;
	register int fd,fl;
	unsigned int dummy,i;
	int inetnum,s,buflen;
	char buf[1000],*sn,meno[500];
	fd_set fs;
...
void weather(UR_OBJECT user, char *inpstr)
{
	...
	filename[0]='\0';
	if (!strncmp(word[1],"za",2))
		strcpy(filename,WEATHER_TODAY_FILE);
	if (!strncmp(word[1],"2",1))
		strcpy(filename,WEATHER_TOMORROW_FILE);
	if (!strncmp(word[1],"ss",2))
		strcpy(filename,WEATHER_STRED_FILE);
	if (!strncmp(word[1],"vs",2))
		strcpy(filename,WEATHER_VYCHOD_FILE);
	if (!strncmp(word[1],"zs",2))
		strcpy(filename,WEATHER_ZAPAD_FILE);
	if (!strncmp(word[1],"ba",2))
		strcpy(filename,WEATHER_BLAVA_FILE);
	if (!strncmp(word[1],"ta",2))
		strcpy(filename,WEATHER_TATRY_FILE);
	if (!filename[0]) {
	...
	switch (more(user,user->socket,filename)) {
		case 0:
			write_user(user,"Prepac, predpoved sa nepodarilo najst.\n");
			break;
		case 1:
			user->misc_op=2;
	}
...
void parsni_pocasie(char vstup[255],char vystfile[255])
{
	FILE *fajl,*vystup;
	char line[101];
	unsigned int stav,i;
	...
	stav=0;
	if (!(fajl=ropen(vstup,"r")))
		return;
	if (!(vystup=ropen(vystfile,"w"))) {
		fclose(fajl);
		return;
	}
	fgets(line,100,fajl);
...
int connecthost(char serveridlo[50],int port)
{
	...
	if ((hp=gethostbyname(serveridlo))==0)
		return -1;
	...
	if (word_count==1) {
		...
		if ((result=mysql_result(query))) {
			...
			while ((row=mysql_fetch_row(result))) {
				...
				subs++;
				if (subs==1)
					write_user(user,title("~OL~FYStrany","~OL~FB"));
		...
		if (!subs && !cnt)
			write_user(user,"Nemas ziadny zaznam v poznamkovom bloku.\n");
		write_user(user,hlp);
		return;
	}
	else {
		...
		if (!strcmp(word[1],"move")) {
			...
			if (subs) {
				...
				if (sub) {
					...
					sprintf(query,"Poznamka bola presunuta na stranu%s.\n",texthb);
		...
		inpstr[512]='\0';
		sprintf(query,"replace into `notes` (`userid`,`subdir`,`note`,`date) values ('%d','%s','%s',FROM_UNIXTIME(%d))",user->id,sub?texthb:"",dbf_string(inpstr),(int)time(0));
		mysql_kvery(query);
		word[1][0]=' ';
		if (sub)
			sprintf(texthb,"na stranu~OL%s~RS",word[1]);
		else
			sprintf(texthb,"do poznamkoveho bloku");
		sprintf(text,"Pridal%s si spravu %s:\n%s\n",pohl(user,"","a"),texthb,inpstr);
		write_user(user,text);
		return;
	}
}
...
void dnote(UR_OBJECT user)
{
	...
	if (od<1 || po<1 || od>po)
		...
		if (deleted) {
			word[1][0]=' ';
			if (sub)
				sprintf(text,"Vygumoval%s si %d sprav%s zo strany%s.\n",pohl(user,"","a"),deleted,skloncislo(deleted,"u","y",""),word[1]);
			else
				sprintf(text,"Vygumoval%s si %d sprav%s z hlavnej strany poznamkoveho bloku.\n",pohl(user,"","a"),deleted,skloncislo(deleted,"u","y",""));
			write_user(user,text);
			return;
		}
		write_user(user,help);
...
int init_remote_connections(UR_OBJECT user)
{
	...
	if (!(rn=ropen(rn_file_name,"r"))) {
		if (user!=NULL)
			write_user(user,"Nemozem otvorit subor vzdialenych pripojeni.\n");
		else {
	...
	if (user==NULL) {
		...
		while (!feof(rn)) {
			if (remote==NULL) {
				...
				write_user(user,"- overwriting remote connection..\n");
			sscanf(line,"%c %s %d %s",&remote->shortcut,remote->name,&remote->port,remote->desc);
			next=remote;
			remote=remote->next;
			fgets(line,199,rn);
	...
	if (user==NULL)
		printf("Inicializujem vzdialeny pristup...\n");
	else
		write_user(user,"Inicializujem vzdialeny pristup...\n");
	return 0;
}
...
int connect_to_site(UR_OBJECT user,RN_OBJECT remote,int slot)
{
	...
	char *sn,serveridlo[100],id[15],filename[255];
	int inetnum,i,def_port;
	FILE *fp;
	...
	reset_alarm();
	if ((user->remote_fds[slot]=ADD_FDS(user->remote_socket[slot],POLLIN|POLLPRI))==0)
		return 4;
...
int exec_remote_com(UR_OBJECT user,char *inpstr)
{
		...
		if (!strncasecmp(&word[0][0],".rem",4) && !strncasecmp("?",&word[1][0],1)) {
			...
			for (j=0;j<MAX_CONNECTIONS;j++)
				if (user->remote_socket[j]==user->actual_remote_socket) {
					remote=user->remote[j];
					break;
				}
			if (remote!=NULL) {
				if (user->remote_socket[j]==user->ircsocknum)
					sprintf(text,"~FTAktivne spojenie: ~OLIRC~RS~FT (~OL~FT%s~RS~FT)\n",get_ircserv_name(user->irc_serv));
				else
					sprintf (text,"~FTAktivny talker: ~OL%s ~RS~FT(~OL%s %d~RS~FT)\n",remote->desc,remote->name,remote->port);
			}
			else
	...
	if ((word[0][0]=='.' && !strncasecmp("rem",&word[0][1],3) && !strncasecmp("nex",&word[1][0],3))) {
		if (user->actual_remote_socket==0)
			j=-1;
		else {
			for (j=0;j<MAX_CONNECTIONS;j++)
	...
	for (remote=remote_start;remote!=NULL;remote=remote->next)
		if ((remote->shortcut==word[0][0] && strlen(inpstr)==1)) {
...
void view_remote(UR_OBJECT user)
{
				...
				if (remote->shortcut==word[2][0] || !strncasecmp(remote->desc,word[2],strlen(word[2]))) {
					...
					switch ((error=connect_to_site(user,remote,slot))) {
						case 1:
							...
							break;
						case 2:
							sprintf(text,"Nemozem najst ziadany server.\n");
							break;
						case 3:
							sprintf(text,"Couldn't connect to %s port %d\n",remote->name,remote->port);
							write_syslog(text,1);
							sprintf(text,"~FTNa ~OL%s~RS~FT sa nepodarilo pripojit.\n",remote->desc);
							break;
						case 4:
							sprintf(text,"Preplnene fds v remote_connect()!\n");
							write_syslog(text,1);
							break;
						case 0:
							i=-1;
	...
	if (!strncmp(word[1],"dis",3)) {
		...
		for (remote=remote_start;remote!=NULL;remote=remote->next)
			if (remote->shortcut==word[2][0] || !strncasecmp(remote->desc,word[2],strlen(word[2])))
				for (j=0;j<MAX_CONNECTIONS;j++)
					if (remote==user->remote[j]) {
						if (user->remote_socket[j]==user->ircsocknum)
							sprintf(text,"~FT[Odpojen%s z ~OLIRC~RS~FT]\n",pohl(user,"y","a"));
						else
							sprintf(text,"~FT[Odpojen%s z ~OL%s %d~RS~FT]\n",pohl(user,"y","a"),user->remote[j]->name,user->remote[j]->port);
						write_user(user,text);
		...
		sprintf(text,"Na taky talker nie si momentalne priojen%s.\n",pohl(user,"y","a"));
		write_user(user,text);
		return;
	}
	...
	if ((rn=ropen(filename2,"r"))) {
		...
		write_syslog("Cannot open remote file in edit_remote_servers().\n",1);
	switch (word_count) {
		case 2:
			sprintf(tmp,"%c",shortcut);
		...
		case 4:
			...
			strcat(query,dbf_string(word[2]));
			strcat(query,"','");
			strcat(query,dbf_string(word[3]));
			strcat(query,"');");
			if (mysql_kvery(query)) {
...
void remote_antiidle()
{
	...
	for (user=user_first;user!=NULL;user=user->next) {
		if (user->type!=USER_TYPE || user->login || user->afk)
			continue;
		for (i=0;i<MAX_CONNECTIONS;i++)
...
extern int spracuj_remote_vstup(char *inpstr)
{
	...
	while ((ptr=strchr(ptr,'\033'))!=NULL) {
		for (x=0;povolene[x][0]!='*';x++)
			if (!strncmp(ptr,povolene[x],strlen(povolene[x])))
				break;
		if (povolene[x][0]=='*')
			*ptr=' ';
		ptr++;
...
extern void parse_telnet_chars(UR_OBJECT user,char *inpstr, size_t len)
{
	...
	sprintf(smalbuf,"%c%c%c%c%c%c%c%c%c%c",IAC,SB,TELOPT_NEW_ENVIRON,'?',NEW_ENV_VAR,'U','S','E','R',NEW_ENV_VALUE);
	if ((user->remtelopt==(user->remtelopt|2)) && strteln(inpstr,smalbuf,10)!=NULL) {
...
extern void parse_remote_ident(UR_OBJECT user,char *rinpstr,int len,int wsock)
{
	...
	if (strteln(rinpstr,smalbuf,len)!=NULL) { /* semraci, semruju ident! */
		...
		write(user->remote_socket[wsock],smalbuf,sajze);
		return;
	}
}
