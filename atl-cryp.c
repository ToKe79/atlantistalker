...
#define l2c(l,c) (*((c)++)=(unsigned char)(((l))&0xff),*((c)++)=(unsigned char)(((l)>>8)&0xff),*((c)++)=(unsigned char)(((l)>>16)&0xff),*((c)++)=(unsigned char)(((l)>>24)&0xff))
...
extern char *crypt_slavko(char *buf,char *salt)
{
	...
	des_cblock key;
	des_key_schedule ks;
	static unsigned char buff[256];  /*CHMEM (20)*/
	unsigned char bb[256]; /*CHMEM (20)*/
	unsigned char *b=bb;
	unsigned char c,u;
 ...
 * Modified by Spartakus, 1999.
 * To Wilder & Somebody: FUCK OFF BOYZ, U HAVE ZERO CHANCE TO CRACK THIS!!!
 */
...
UINT4 *DECODE(unsigned char *x)
{
	...
	for (i=0;i<14;i++) {
		y[i]=(UINT4)x[0]+((UINT4)x[1]<<8)+((UINT4)x[2]<<16)+((UINT4)x[3]<<24);
		x+=4;
	}
	y[14]=*(UINT4 *)x;
	return y;
...
void to64(char *s,UINT4 v,int n)
{
	while (--n) {
		*s++=itoa64[v&0x3F];
		v>>=6;
...
extern char *md5_crypt(char *pw,char *salt)
{
	...
	if (!strncmp(sp=salt,magic,3))
		sp+=3;
	ep=sp;
	while (*ep && *ep!='$' && ep<sp+8)
		ep++;
	if ((len_p=strlen(pw))>15)
		len_p=15;
	memcpy(odd_p,pw,len_p);
	memcpy(odd_s,sp,len_s=ep-sp);
	memcpy(odd_ps,odd_p,len_p);
	memcpy(&odd_ps[len_p],odd_s,len_s);
	len_ps=len_p+len_s;
	memcpy(odd_pp,odd_p,len_p);
	memcpy(&odd_pp[len_p],odd_p,len_p);
	len_pp=len_p<<1;
	memcpy(odd_psp,odd_ps,len_ps);
	memcpy(&odd_psp[len_ps],odd_p,len_p);
	len_psp=len_ps+len_p;
	memcpy(even_p,odd_p,len_p);
	memcpy(&even_p[len_p],PADDING,40-len_p);
	*(UINT4 *)&even_p[40]=(len_p+16)<<3;
	memcpy(even_sp,odd_s,len_s);
	memcpy(&even_sp[len_s],odd_p,len_p);
	...
	*(UINT4 *)&even_pp[40]=(len_pp+16)<<3;
	memcpy(even_spp,odd_s,len_s);
	memcpy(&even_spp[len_s],odd_pp,len_pp);
