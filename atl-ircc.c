...
int get_charclient_line(UR_OBJECT user,char *inpstr,int len)
{
	...
	for(l=0;l<len;++l) {
		...
		if (user->statline==CHARMODE && inpstr[l]==9) {
			if (user->actual_remote_socket!=0 && user->actual_remote_socket==user->ircsocknum)
				...
				sprintf(user->buff,".tell %s ",user->lasttellfrom);
		...
		if ((inpstr[l]>0 && inpstr[l]<32) || user->buffpos+2==BUFSIZE) {
			...
			if (user->charmode_echo && user->statline!=CHARMODE)
				write_user(user,"\n");
			else if (user->statline==CHARMODE)
	...
	if (user->charmode_echo && user->statline!=CHARMODE && ((user->login!=2 && user->login!=1) || password_echo))
...
void statline(UR_OBJECT user)
{
	if (user->akl || user->socket>1000) {
		write_user(user,"Prepac, tato funkcia nieje dostupna ked sa pripajas cez Atlantis Klienta alebo web.\n");
		return;
	}
...
void load_irc_details(UR_OBJECT user)
{
	...
	if (ok==1) {
		if ((row=mysql_fetch_row(result))) {
			if (row[0])
				strcpy(user->irc_defnick,row[0]);
			else
				strcpy(user->irc_defnick,user->name);
			if (row[1])
				strcpy(user->irc_chan,row[1]);
			else
				strcpy(user->irc_chan,"#atlantis");
			if (row[2])
				strcpy(user->irc_serv,row[2]);
			else
...
{
	if (user->irc_defnick[0]=='\0' || user->irc_name[0]=='\0')
		return;
	if (user->level<SAG)
		return;
	if (user->irc_chan[0]=='\0')
		strcpy(user->irc_chan,"#atlantis");
	if (user->irc_serv[0]=='\0')
		strcpy(user->irc_serv,"nextra1");
	sprintf(query,"replace into `irc` (`userid`,`defnick`,`channel`,`server`,`name`) values ('%d','",user->id);
	strcat(query,dbf_string(user->irc_defnick));
	strcat(query,"','");
	strcat(query,dbf_string(user->irc_chan));
	strcat(query,"','");
	strcat(query,dbf_string(user->irc_serv));
	strcat(query,"','");
	strcat(query,dbf_string(user->irc_name));
	strcat(query,"');");
	mysql_kvery(query);
}
...
void irc_prikaz(UR_OBJECT user, char *inpstr)
{
	...
	if (!strncmp(word[0],"/ms",3) || !strncmp(word[0],"/pr",3) || !strcmp(word[0],"/m")) {
		...
		if (word[1][0]=='#' || word[1][0]=='&')
			...
			sprintf(text,"~OL~FK[~RS~FRmsg(->~OL~FR%s~RS~FR)~OL~FK]~RS~FW %s\n",word[1],inpstr);
		record_irc(user,text);
		write_user(user,text);
	}
	/* INFO */
...
void prihlas_irc(UR_OBJECT user)
{
	...
	if (user->remote_login_socket==0)
		return;
	user->irc_reg=0;
	for (i=0; i<10; i++)
		user->channels[i][0]='\0';
	user->irc_chan[0]='\0';
	if (user->irc_name[0]=='\0')
		sprintf(user->irc_name,"%s z Atlantidy",user->name);
...
void spracuj_irc_vstup(UR_OBJECT user,char *inpstr)
{
	...
	if (!strcmp(irc_word[curw],"NOTICE")) {
		if (rnick[0])
			...
			sprintf(text,"~OL~FG!~RS~FG%s ~RS~FW%s\n",&irc_word[0][1],irc_word[lastw]);
	}
	...
	if (!strcmp(irc_word[curw],"PRIVMSG")) {
		if (irc_word[curw+1][0]!='#' && irc_word[curw+1][0]!='&')
			...
			sprintf(text,"~OL~FB<~RS~FW%s:~FT%s~OL~FB>~RS~FW %s\n",irc_word[curw+1],rnick,irc_word[lastw]);
	}
	...
	if (!strncmp(irc_word[curw],"31",2) || !strcmp(irc_word[curw],"301") || !strcmp(irc_word[curw],"369")) {  /* /WHOIS */
		...
		if (irc_word[curw][2]=='9')
			sprintf(text,"~OL~FB-> ~RS~FWChannels : %s\n",irc_word[lastw]);
		if (irc_word[curw][2]=='2')
			sprintf(text,"~OL~FB-> ~RS~FWServer   : ~OL%s ~RS~FW(%s)\n",irc_word[curw+3],irc_word[lastw]);
		if (irc_word[curw][1]=='0')
			sprintf(text,"~OL~FB-> ~RS~FWAway     : %s\n",irc_word[lastw]);
		if (irc_word[curw][2]=='3')
			sprintf(text,"~OL~FB-> ~RS~FWOperator : %s is an IRC operator.\n",irc_word[curw+2]);
		if (irc_word[curw][2]=='4')
			sprintf(text,"~OL~FB-> ~FW%s ~RS~FWwas (%s@%s)\n~OL~FB-> ~RS~FWIRCname  : %s\n",irc_word[curw+2],irc_word[curw+3],irc_word[curw+4],irc_word[lastw]);
		if (irc_word[curw][2]=='8' || irc_word[curw][1]=='6')
			sprintf(text,"~OL~FB-> =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
		if (irc_word[curw][2]=='7') {
			int since_int,idle;
			if (irc_word[curw+4][0])
				since_int=atoi(irc_word[curw+4]);
			else
				since_int=0;
			idle=atoi(irc_word[curw+3]);
			if (since_int)
				sprintf(text,"~OL~FB-> ~RS~FWIdle     : ~OL%d~RS day ~OL%d~RS hr, ~OL%d~RS min (login: %s)\n",idle/86400,(idle%86400)/3600,(idle%3600)/60,zobraz_datum((time_t *)&since_int,1));
			else
				sprintf(text,"~OL~FB-> ~RS~FWIdle     : ~OL%d~RS day ~OL%d~RS hr, ~OL%d~RS min\n",idle/86400,(idle%86400)/3600,(idle%3600)/60);
		}
	}
	if (!strcmp(irc_word[curw],"KICK")) {
		if (irc_word[lastw][0])
			sprintf(text,"~OL~FB=~FW*~FB= ~OL~FW%s~RS was kicked off %s by ~FT%s ~FR(~OL~FR%s~RS~FR)\n",irc_word[curw+2],irc_word[curw+1],rnick,irc_word[lastw]);
		else
			sprintf(text,"~OL~FB=~FW*~FB= ~OL~FW%s~RS was kicked off %s by ~FT%s~RS\n",irc_word[curw+2],irc_word[curw+1],rnick);
		if (!strcmp(irc_word[curw+2],user->irc_nick)) {
			for (i=0; i<10; i++)
				if (!strcmp(user->channels[i],irc_word[curw+1])) {
					user->channels[i][0]='\0';
					break;
				}
	...
	if (!strcmp(irc_word[curw],"332"))
		sprintf(text,"~OL~FB=~FW*~FB= ~RS~FWTopic for ~FT%s~FW:~OL %s~RS\n",irc_word[curw+2],irc_word[lastw]);
	...
	if (!strcmp(irc_word[curw],"PART")) {
		if (irc_word[curw+2][0]>32)
			...
			sprintf(text,"~OL~FB=~FW*~FB= ~RS~FW%s has left channel %s\n",rnick,irc_word[curw+1]);
		if (!strcmp(rnick,user->irc_nick)) {
			for (i=0; i<10; i++)
				if (!strcmp(user->channels[i],irc_word[curw+1])) {
					user->channels[i][0]='\0';
	...
	if (!strcmp(irc_word[curw],"MODE")) {
		if (!irc_word[curw+3][0])
			...
			sprintf(text,"~OL~FB=~FW*~FB= ~RS~FWmode ~FT%s ~FW[~OL%s %s~RS] by ~OL%s~RS\n",irc_word[curw+1],irc_word[curw+2],irc_word[curw+3],rnick);
		else
	...
	if (!strcmp(irc_word[curw],"402"))
		sprintf(text,"~OL%s~RS: no such server.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"403"))
		sprintf(text,"~OL%s~RS: no such channel.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"404"))
		sprintf(text,"~OL%s~RS: cannot send to channel.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"405"))
		sprintf(text,"~OL%s~RS: you have joined too many channels.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"406"))
		sprintf(text,"~OL%s~RS: there was no such nickname.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"407"))
		sprintf(text,"~OL%s~RS: duplicate recipients.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"411"))
		sprintf(text,"no recipient given.\n");
	if (!strcmp(irc_word[curw],"412"))
		sprintf(text,"no text to send.\n");
	if (!strcmp(irc_word[curw],"413"))
		sprintf(text,"~OL%s~RS: no toplevel domain specified.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"414"))
		sprintf(text,"~OL%s~RS: wildcard in toplevel domain.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"421"))
		sprintf(text,"~OL%s~RS: unknown command.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"422"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW MOTD file is missing.\n");
	if (!strcmp(irc_word[curw],"423"))
		sprintf(text,"~OL%s~RS: no administrative info available.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"423"))
		sprintf(text,"file error.\n");
	if (!strcmp(irc_word[curw],"431"))
		sprintf(text,"no nickname given.\n");
	if (!strcmp(irc_word[curw],"432"))
		sprintf(text,"~OL%s~RS: invalid nickname.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"433"))
		sprintf(text,"~OL%s~RS: nick already in use.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"436"))
		sprintf(text,"~OL%s~RS: nickname collision KILL.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"438"))
		sprintf(text,"~OL%s~RS: %s\n",irc_word[curw+2],irc_word[lastw]);
	if (!strcmp(irc_word[curw],"441"))
		sprintf(text,"~OL%s/%s~RS: they aren't on that channel.\n",irc_word[curw+3],irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"442"))
		sprintf(text,"~OL%s~RS: you aren't on that channel.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"443"))
		sprintf(text,"~OL%s/%s~RS: is already on channel.\n",irc_word[curw+3],irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"444"))
		sprintf(text,"~OL%s~RS: user not logged in.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"445"))
		sprintf(text,"SUMMON has been disabled.\n");
	if (!strcmp(irc_word[curw],"446"))
		sprintf(text,"USERS has been disabled.\n");
	if (!strcmp(irc_word[curw],"451")) {
		sprintf(text,"you have not registered.\n");
		sprintf(text2,"Pouzi: /nick <novy nick>\n");
	}
	if (!strcmp(irc_word[curw],"461"))
		sprintf(text,"~OL%s~RS: not enough parameters.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"462"))
		sprintf(text,"you may not reregister.\n");
	if (!strcmp(irc_word[curw],"463"))
		sprintf(text,"your host isn't among the privileged.\n");
	if (!strcmp(irc_word[curw],"464"))
		sprintf(text,"password incorrect.\n");
	if (!strcmp(irc_word[curw],"465"))
		sprintf(text,"you are banned from this server.\n");
	if (!strcmp(irc_word[curw],"467"))
		sprintf(text,"~OL%s~RS: channel key already set.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"471"))
		sprintf(text,"~OL%s~RS: cannot join channel (limited channel).\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"472"))
		sprintf(text,"~OL%s~RS: is unknown mode char to me.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"473"))
		sprintf(text,"~OL%s~RS: cannot join channel (invite only)\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"474"))
		sprintf(text,"~OL%s~RS: cannot join channel (banned from channel!)\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"475"))
		sprintf(text,"~OL%s~RS: cannot join channel (key required!)\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"481"))
		sprintf(text,"Permission Denied- You're not an IRC operator.\n");
	if (!strcmp(irc_word[curw],"482"))
		sprintf(text,"~OL%s~RS: you're not channel operator.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"483"))
		sprintf(text,"you can't kill a server.\n");
	if (!strcmp(irc_word[curw],"491"))
		sprintf(text,"No O-lines for your host.\n");
	if (!strcmp(irc_word[curw],"501"))
		sprintf(text,"Unknown MODE flag.\n");
	if (!strcmp(irc_word[curw],"502"))
		sprintf(text,"Cant change mode for other users.\n");
	if (!strcmp(irc_word[curw],"303"))
		sprintf(text,"~OL~FB=~FW*~FB= ~RS~FWCurrently online: %s\n",irc_word[lastw]);
	if (!strcmp(irc_word[curw],"305"))
		sprintf(text,"You're no longer marked as being away.\n");
	if (!strcmp(irc_word[curw],"306"))
		sprintf(text,"You have been marked as being away.\n");
	if (!strcmp(irc_word[curw],"321"))
		sprintf(text,"~OL~FWChannel              #Users Topic\n");
	if (!strcmp(irc_word[curw],"322"))
		sprintf(text,"~OL~FY%-20s   ~RS~FT%-3s   %s\n",irc_word[curw+2],irc_word[curw+3],irc_word[lastw]);
	if (!strcmp(irc_word[curw],"323"))
		sprintf(text,"~OL~FW---\n");
	...
	if (!strcmp(irc_word[curw],"342"))
		sprintf(text,"~OL%s~RS: summoning user to IRC.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"351"))
		sprintf(text,"~OL%s~RS: %s (~FT%s~FW)\n",irc_word[curw+3],irc_word[curw+2],irc_word[lastw]);
	if (!strcmp(irc_word[curw],"353"))
		sprintf(text,"~OL~FB=~FW*~FB= ~RS~FWUsers on ~FG[~OL~FG%s~RS~FG]~FW: ~OL~FB%s\n",irc_word[curw+3],irc_word[lastw]);
	if (!strcmp(irc_word[curw],"371"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW %s\n",irc_word[lastw]);
	if (!strcmp(irc_word[curw],"374"))
		sprintf(text,"~FB[~OL~FY*~RS~FB] =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
	if (!strcmp(irc_word[curw],"381"))
		sprintf(text,"You are now an IRC operator\n");
	if (!strcmp(irc_word[curw],"391"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW %s\n",irc_word[lastw]);
	if (!strcmp(irc_word[curw],"252"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW ~OL%s~RS operator(s) online.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"253"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW ~OL%s~RS unknown connection(s).\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"254"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW ~OL%s~RS channels formed.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"254"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW ~OL%s~RS channels formed.\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"256"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW ~OL%s~RS Administrative info\n",irc_word[curw+2]);
	if (!strcmp(irc_word[curw],"265"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW %s\n",irc_word[lastw]);
	if (!strcmp(irc_word[curw],"266"))
		sprintf(text,"~FB[~OL~FY*~RS~FB]~FW %s\n",irc_word[lastw]);
	record_irc(user,text);
	if (!user->ignall)
		write_user(user,text);
...
void clear_irc_words(void)
{
	int w;
	for(w=0;w<MAX_IRC_WORDS;++w)
		irc_word[w][0]='\0';
}
